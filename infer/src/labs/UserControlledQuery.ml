(** UserControlledQuery checker for detecting SQL injection through user-controlled sources *)

open! IStd
module F = Format
module L = Logging

module Domain = UserControlledQueryDomain
...
(** Check if a method is a SQL query sink *)
let is_sql_query_sink proc_name =
  match proc_name with
  | Procname.Java java_proc_name ->
    let class_name = Procname.Java.get_class_name java_proc_name in
    let method_name = Procname.Java.get_method java_proc_name in
    (* JDBC Statement methods *)
    (String.is_substring class_name ~substring:"Statement" ||
     String.is_substring class_name ~substring:"PreparedStatement") &&
    (String.equal method_name "executeQuery" ||
     String.equal method_name "executeUpdate" ||
     String.equal method_name "execute") ||
    (* JPA EntityManager methods *)
    String.is_substring class_name ~substring:"EntityManager" &&
    (String.equal method_name "createQuery" ||
     String.equal method_name "createNativeQuery") ||
    (* MyBatis methods *)
    String.is_substring class_name ~substring:"SqlSession" &&
    (String.equal method_name "selectList" ||
     String.equal method_name "selectOne" ||
     String.equal method_name "insert" ||
     String.equal method_name "update" ||
...
module TransferFunctions (CFG : ProcCfg.S) = struct
  ...
  type analysis_data = UserControlledQueryDomain.t InterproceduralAnalysis.t
  ...
  let exec_instr (astate : Domain.t) analysis_data _node _instr_index (instr : Sil.instr) =
    ...
    let report_error loc message =
      Reporting.log_issue proc_desc err_log ~loc UserControlledQuery IssueType.user_controlled_query message
    in
    
    (* Debug: log all Call instructions *)
    (match instr with
    | Call (_, Const (Cfun callee_proc_name), _, _, _) ->
        L.debug Analysis Verbose "UserControlledQuery: Call to %s@." (Procname.to_string callee_proc_name)
    | _ -> ());
    ...
    | Call ((return_id, _), Const (Cfun callee_proc_name), actuals, loc, _call_flags) ->
      ...
      ) else if is_sql_query_sink callee_proc_name then (
        (* This is a SQL query execution - check if any argument is tainted *)
        if has_tainted_argument actuals astate then (
          let message = "Query built from user-controlled sources" in
    ...
    | Call (_, Const (Cfun callee_proc_name), actuals, loc, _call_flags) ->
      ...
      if is_sql_query_sink callee_proc_name then (
        (* This is a SQL query execution - check if any argument is tainted *)
        if has_tainted_argument actuals astate then (
          let message = "Query built from user-controlled sources" in
          report_error loc message
  ...
  let pp_session_name _node fmt = F.pp_print_string fmt "user controlled query"
end
