(** Abstract domain for SSRF (Server-Side Request Forgery) checker *)

open! IStd
module F = Format

(** Set of tainted variables that may contain user-controlled URLs *)
...
(** Abstract domain state for SSRF detection *)
type t = {
  tainted_vars : TaintedVarSet.t;      (* Variables containing tainted URL data *)
...
let pp fmt astate =
  F.fprintf fmt "SSRF Domain: tainted_vars=%a, tainted_sources=%a"
